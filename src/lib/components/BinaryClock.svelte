<script lang="ts">
  ...
  const effectiveInterval = $derived(
    mode === 'noise'                              ?  50 :
    mode === 'lissajous' || mode === 'analog'     ? 100 :
    baseInterval[msPrecision]
  );
  ...
  // ── Lissajous ──────────────────────────────────────────────────────────────
  function lissajousPath(a: number, b: number, phase: number, r: number, cx: number, cy: number): string {
    const n = 600;
    let d = '';
    for (let i = 0; i <= n; i++) {
      const t = (i / n) * 2 * Math.PI;
      const x = (cx + r * Math.sin(a * t + phase)).toFixed(2);
      const y = (cy + r * Math.sin(b * t)).toFixed(2);
      d += i === 0 ? `M${x},${y}` : `L${x},${y}`;
    }
    return d;
  }
  ...
  function noiseAt(x: number, y: number, t: number): number {
    return Math.max(0, Math.min(1,
      (Math.sin(x * 1.1 + t * 0.4) * Math.cos(y * 0.8 - t * 0.3) +
       Math.sin((x + y) * 0.6 + t * 0.7) * 0.5 + 1.5) / 3
    ));
  }

  // ── Analog helpers ─────────────────────────────────────────────────────────
  function px(cx: number, r: number, deg: number) { return cx + r * Math.cos((deg - 90) * Math.PI / 180); }
  function py(cy: number, r: number, deg: number) { return cy + r * Math.sin((deg - 90) * Math.PI / 180); }
  ...
  let t       = $derived(now.getTime() / 1000);
  ...
  // Lissajous
  let lissSize  = 220;
  let lissA     = $derived((now.getHours()   % 7) + 1);
  let lissB     = $derived((now.getMinutes() % 7) + 1);
  let lissPhase = $derived((now.getSeconds() + now.getMilliseconds() / 1000) / 60 * 2 * Math.PI);

  // Analog
  const anaSize = 200;
  const anaR    = 88;
  const anaCX   = anaSize / 2;
  const anaCY   = anaSize / 2;
  let hAngle = $derived(((now.getHours() % 12) + now.getMinutes() / 60) / 12 * 360);
  let mAngle = $derived((now.getMinutes() + now.getSeconds() / 60) / 60 * 360);
  let sAngle = $derived((now.getSeconds() + now.getMilliseconds() / 1000) / 60 * 360);
...
{:else if mode === 'dots'}
  <div
    class="select-none {wrapperClass[orientation]} {showBg ? 'bg-black/30 px-5 py-4 rounded' : ''}"
    aria-label="Dot clock" role="timer"
  >
    {#each columns as col}
      <div class={colClass[orientation]}>
        {#each toBits(col.value, col.bits) as bit}
          <div style="
            width:{squareSize}px;height:{squareSize}px;border-radius:50%;
            {bit
              ? `background-color:${colorOn};mix-blend-mode:${blendMode};${glowSize > 0 ? `box-shadow:0 0 ${glowSize}px ${colorOn}` : ''}`
...
{:else if mode === 'lissajous'}
  <svg width={lissSize} height={lissSize} aria-label="Lissajous clock" role="timer">
    <rect width={lissSize} height={lissSize} fill={colorOff} opacity="0.15" rx="2" />
    <path
      d={lissajousPath(lissA, lissB, lissPhase, lissSize / 2 - 8, lissSize / 2, lissSize / 2)}
      fill="none" stroke={colorOn} stroke-width="1.5"
      style={svgGlow ? `filter:${svgGlow}` : ''} />
  </svg>

<!-- ─── Analog ────────────────────────────────────────────────────────────── -->
{:else if mode === 'analog'}
  <svg width={anaSize} height={anaSize} aria-label="Analog clock" role="timer">
    <!-- Face -->
    <circle cx={anaCX} cy={anaCY} r={anaR} fill="none" stroke={colorOff} stroke-width="1" opacity="0.4" />
    <!-- Hour ticks -->
    {#each Array(12) as _, i}
      {@const deg = i * 30}
      <line
        x1={px(anaCX, anaR - 6, deg)} y1={py(anaCY, anaR - 6, deg)}
        x2={px(anaCX, anaR,     deg)} y2={py(anaCY, anaR,     deg)}
        stroke={colorOff} stroke-width="1.5" opacity="0.5" />
    {/each}
    <!-- Hour hand -->
    <line x1={anaCX} y1={anaCY}
      x2={px(anaCX, anaR * 0.5, hAngle)} y2={py(anaCY, anaR * 0.5, hAngle)}
      stroke={colorOn} stroke-width="3" stroke-linecap="round"
      style={svgGlow ? `filter:${svgGlow}` : ''} />
    <!-- Minute hand -->
    <line x1={anaCX} y1={anaCY}
      x2={px(anaCX, anaR * 0.75, mAngle)} y2={py(anaCY, anaR * 0.75, mAngle)}
      stroke={colorOn} stroke-width="2" stroke-linecap="round"
      style={svgGlow ? `filter:${svgGlow}` : ''} />
    <!-- Second hand -->
    <line x1={anaCX} y1={anaCY}
      x2={px(anaCX, anaR * 0.88, sAngle)} y2={py(anaCY, anaR * 0.88, sAngle)}
      stroke={colorOn} stroke-width="1" stroke-linecap="round"
      style={svgGlow ? `filter:${svgGlow}` : ''} />
    <!-- Center dot -->
    <circle cx={anaCX} cy={anaCY} r="3" fill={colorOn}
      style={svgGlow ? `filter:${svgGlow}` : ''} />
  </svg>
...
{:else if mode === 'noise'}
  <svg
    ...
    aria-label="Noise clock" role="timer"
  >
    {#each Array(NROWS) as _, row}
      {#each Array(NCOLS) as _, col}
        {@const n = noiseAt(col, row, t)}
        <rect
        ...
        <rect
          x={col * squareSize} y={row * squareSize}
          width={squareSize}   height={squareSize}
          fill={colorOn} opacity={n}
          style={glowSize > 0 && n > 0.6 ? `filter:drop-shadow(0 0 ${glowSize * n}px ${colorOn})` : ''} />
      {/each}
    {/each}
  </svg>
{/if}
